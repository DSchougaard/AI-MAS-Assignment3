...
\section{Experiments and Results}
	...
	\subsection{Goal Dependency}
		...
		%and the agent therefore can't easily move around previously achieved goals and end up undoing them instead, and thereby resulting in a longer solution.
		...
		%This is due to the most time consuming part of of the level is in practice a minor version of the single agent level \verb|SAboxesOfHanoi| and a performance more similar to pattern from the single agent levels are therefore expected.
...
\section{Future Work}
	...
	%	Our approach has some general disadvantages as discussed in section \ref{disscussion} and as shown in the competition results, the quality of the single agent solution could be higher. These areas would be obvious to address, but to do this a completely new approach is need
		...
		\label{subsub:backtrack}
			Performing the search completely offline, will allow better backtracking. This will allow us to recover from situations, where the agent would otherwise get stuck, by solving the goal in the wrong order.
